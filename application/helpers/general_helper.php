...
function _l($line, $label = '', $log_errors = true)
{
    ...
    if ($line !== null && mb_strpos($line, '_db_') !== false) {
...
/**
 * Get the current user's branch (workplace) from tblworkplace
 *
 * @since  Version 3.x
 *
 * @return string Branch name or 'n/a' if not available
 */
function get_git_branch()
{
    ...
    // First, try to get workplace_id from session (faster)
    $workplace_id = $CI->session->userdata('staff_workplace_id');
    
    if (!empty($workplace_id) && is_numeric($workplace_id) && $workplace_id > 0) {
        // Query workplace directly using session value
        $CI->db->select('workplace_name');
        $CI->db->from(db_prefix() . 'workplace');
        $CI->db->where('workplace_id', (int)$workplace_id);
        $query = $CI->db->get();
        
        if ($query->num_rows() > 0) {
            $result = $query->row();
            if (!empty($result->workplace_name)) {
                return $result->workplace_name;
            }
        }
    }
    
    // Query using join: select * from tblstaff t join tblworkplace w on w.workplace_id = t.workplace
    $CI->db->select('w.workplace_name');
    $CI->db->from(db_prefix() . 'staff t');
    $CI->db->join(db_prefix() . 'workplace w', 'w.workplace_id = t.workplace', 'inner');
    $CI->db->where('t.staffid', $staff_id);
    $query = $CI->db->get();
    ...
    if ($query->num_rows() > 0) {
        ...
        if (!empty($result->workplace_name)) {
            ...
            $CI->db->select('workplace');
            $CI->db->from(db_prefix() . 'staff');
            $CI->db->where('staffid', $staff_id);
            $staff_query = $CI->db->get();
            if ($staff_query->num_rows() > 0) {
                $staff_result = $staff_query->row();
                if (!empty($staff_result->workplace)) {
                    $CI->session->set_userdata('staff_workplace_id', (int)$staff_result->workplace);
                }
            }
            return $result->workplace_name;
        }
