...
class Reminder_model extends App_Model
{
	...
	{
		return $this->db->query('SELECT DISTINCT(staff) as sale_agent FROM ' . db_prefix() . 'reminders WHERE staff != 0')->result_array();
	}
	...
	{
		return $this->db->query('SELECT DISTINCT('. db_prefix() .'reminders.rel_id) as userid, '. db_prefix() .'clients.company  FROM ' . db_prefix() . 'reminders JOIN '. db_prefix() .'clients ON '. db_prefix() .'clients.userid='. db_prefix() . 'reminders.rel_id WHERE '. db_prefix() . 'reminders.rel_type="customer"')->result_array();
	}
	...
	{ 
		...
		$data['notify_by_call_client'] = isset($data['notify_by_call_client']) ? '2' : '0';
		...
		// Map customer to rel_id if customer is provided and rel_type is customer
		if (isset($data['customer']) && !empty($data['customer'])) {
			if (empty($data['rel_type']) || $data['rel_type'] == 'customer') {
				$data['rel_id'] = $data['customer'];
				$data['rel_type'] = 'customer';
			}
			unset($data['customer']); // Remove customer key as column doesn't exist
		}
		$data['staff'] =$data['assigned_to'];
		// Note: created_by_staff column doesn't exist in reminders table
	...
	{
		$this->db->select(db_prefix().'reminders.id,'.db_prefix().'reminders.description,'.db_prefix().'reminders.date,'.db_prefix().'reminders.rel_type,'.db_prefix().'reminders.rel_id,'.db_prefix().'reminders.contact,'.db_prefix().'reminders.rel_id as customer,'.db_prefix().'contacts.email,'.db_prefix().'contacts.phonenumber,'.db_prefix().'reminders.custom_recurring,'.db_prefix().'reminders.recurring,'.db_prefix().'reminders.cycles,'.db_prefix().'reminders.total_cycles,'.db_prefix().'reminders.staff as assigned_to,'.db_prefix().'reminders.notify_by_email,'.db_prefix().'reminders.repeat_every,'.db_prefix().'reminders.recurring_type,'.db_prefix().'reminders.is_complete,'.db_prefix().'clients.company,'.db_prefix().'reminders.notify_by_email_client,'.db_prefix().'reminders.notify_by_sms_client,'.db_prefix().'reminders.isnotified')->from(db_prefix().'reminders');
		$this->db->join(db_prefix().'contacts',db_prefix().'reminders.contact='.db_prefix().'contacts.id','left');
		$this->db->join(db_prefix().'clients',db_prefix().'reminders.rel_id='.db_prefix().'clients.userid AND '.db_prefix().'reminders.rel_type="customer"','left');
	...
	{
		$this->db->where('id', $id);
		$this->db->update(db_prefix() . 'reminders',['is_complete'=>'0','isnotified'=>'1']);
		if ($this->db->affected_rows() > 0) {
	...
	public function add_comment($data, $client = false)
	{
        ...
        $comment = $data['content'].". Pledge Date: ".$data['pledge_date'].", Amount Recovered: ".$data['amount_recovered'].', Pledge Amount: '.$data['pledge_amount'];
		$this->log_reminder_activity($data['reminder_id'], 'reminder_comment_added',false,serialize([$comment]));
		return true;
	}
	...
	{
		// Note: created_by_staff column doesn't exist in reminders table
		// Return empty array since this feature is not available
		return [];
		// Original query (commented out due to missing column):
