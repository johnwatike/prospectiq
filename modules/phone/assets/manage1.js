...
$(function(){
   ...
   var tAPI=  initDataTable('.table-reminder', admin_url+'phone/table', ['undefined'], ['undefined'], reminder_ServerParams, [0, 'desc']);
   init_reminder();
...
function init_reminder(id) {
    reminder_load_small_table_item(id, '#reminder', 'reminderid', 'phone/get_reminder_data_ajax', '.table-reminder');
}
...
$('body').on('change','#rel_type', function() {
  ...
  $.get(admin_url + 'phone/get_related_doc/' + rel_type+'/'+_customer_id, function(response) {
      if(response){
...
$('body').on('change','#customer', function() {
    _customer_id = $("#customer").val() ? $("#customer").val() : 0;
    $('select[name=rel_type]').val(1);
    $('.selectpicker').selectpicker('refresh')
    if(_customer_id != ''){
      $.get(admin_url + 'phone/get_contact_data_values/' + _customer_id + '/customer', function(response) {
          $('.proposal_to_wrap').html(response.field_to);
          $('#contact').selectpicker('refresh');
          $('.proposal_to_wrap').removeClass('hide');
      }, 'json');
    }else{
    ...
    $('.proposal_to_wrap').addClass('hide');
    $('#rel_type').eq(1).prop('selected', true);
