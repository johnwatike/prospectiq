...
public function latch_payslip(){
 ...
 public function delete_payroll_type($id)
    ...
    public function update_staff_workplace()
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        }
        ...
        if (empty($workplace_id) || !is_numeric($workplace_id)) {
            echo json_encode([
                'success' => false,
                'message' => _l('form_validation_required') . ' ' . _l('workplace')
            ]);
            return;
        }

        // Validate workplace exists
        $workplace = $this->hrm_model->get_workplace();
        $workplace_exists = false;
        foreach ($workplace as $wp) {
            if ($wp['workplace_id'] == $workplace_id) {
                $workplace_exists = true;
                break;
            }
        }
        ...
        if (!$workplace_exists) {
            echo json_encode([
                'success' => false,
                'message' => _l('workplace') . ' ' . _l('not_found')
            ]);
            return;
        }

        // Update staff workplace
        ...
        if ($success) {
            // Update session
            $this->session->set_userdata('staff_workplace_id', (int)$workplace_id);
            ...
            echo json_encode([
                'success' => false,
                'message' => _l('problem_updating', _l('workplace'))
            ]);
        }
    }
