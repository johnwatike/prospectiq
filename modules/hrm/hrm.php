...
hooks()->add_action('app_admin_footer', 'hrm_include_workplace_dialog');
...
function hrm_add_footer_components(){
	...
	// Load workplace selection dialog script for all admin pages if user is staff
	if (is_staff_logged_in()) {
		echo '<script src="'.module_dir_url('hrm', 'assets/js/workplace_selection.js').'"></script>';
	}
...
function hrm_include_workplace_dialog() {
	$CI = &get_instance();
	if (is_staff_logged_in()) {
		// Load the model before loading the view (use module path format)
		...
		if ($workplace_id === null || $workplace_id === false || $workplace_id == 0) {
			$staff_id = get_staff_user_id();
			if ($staff_id) {
				// Query database to get workplace from staff table
				$CI->db->select('t.workplace');
				...
				if ($query->num_rows() > 0) {
					$result = $query->row();
					$workplace_id = !empty($result->workplace) ? (int)$result->workplace : 0;
					// Update session for next time
					$CI->session->set_userdata('staff_workplace_id', $workplace_id);
				} else {
					$workplace_id = 0;
				}
			} else {
		...
		// Pass workplace_id to JavaScript (0 or null means no workplace assigned)
		echo '<script>var staff_workplace_id = ' . (int)$workplace_id . ';</script>';
		
		// Include dialog view
		$CI->load->view('hrm/includes/workplace_selection_dialog');
	}
}
