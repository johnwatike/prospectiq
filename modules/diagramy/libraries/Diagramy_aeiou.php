...
use Firebase\JWT\JWT as Diagramy_JWT;
use Firebase\JWT\Key as Diagramy_Key;
use WpOrg\Requests\Requests as Diagramy_Requests;

class Diagramy_aeiou
{
    public static function getPurchaseData($code)
    {
        $givemecode = Diagramy_Requests::get(GIVE_ME_CODE)->body;
        ...
        $response   = Diagramy_Requests::get($verify_url.'?code='.$code, $headers, $options);
    ...
    public function validatePurchase($module_name)
    {
        ...
        if (4 == count($id_data)) {
            ...
            $data     = Diagramy_JWT::decode($token, new Diagramy_Key($id_data[3], 'HS512'));
            $verified = !empty($data)
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request  = Diagramy_Requests::post(VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
