...
function _apply_menu_items_position($items, $options)
{
    if (!is_array($options)) {
        ...
        foreach ($items as $key => $item) {
            ...
            if (isset($item['collapse'])) {
                if (!isset($newItems[$key]['children'])) {
                    $newItems[$key]['children'] = [];
                }
                
                if (isset($item['children']) && is_array($item['children'])) {
                    foreach ($item['children'] as $childKey => $child) {
                        if (isset($child['slug']) && !in_array($child['slug'], $newItemsAddedKeys)) {
                            $newItems[$key]['children'][] = $child;
                        }
                    }
                }

                if (isset($newItems[$key]['children']) && is_array($newItems[$key]['children'])) {
    ...
    foreach ($items as $key => $item) {
        ...
        if (isset($options->{$item['slug']})) {
            ...
            if (isset($item['children']) && is_array($item['children'])) {
                foreach ($item['children'] as $childKey => $child) {
                    if (isset($child['slug']) && isset($options->{$item['slug']}->children->{$child['slug']})) {
